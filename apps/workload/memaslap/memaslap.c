...
int main(int argc, char *argv[])
{
  ...
  qsort(ms_statistic.total_stat.all_times_lt, 10000000, sizeof(uint64_t), compare );

  uint64_t cdfArray[101];
  cdfArray[0] = ms_statistic.total_stat.all_times_lt[0];
  int i = 1;
  for (i = 1; i < 101; i++)
  {
    int idx = 10000000 * 0.01 * i - 1;
    cdfArray[i] = ms_statistic.total_stat.all_times_lt[idx];
  }
